...
if($interactive) {
	...
	if ($node_opts{gpu_cnt}) {
		$node_opts{gpu_cnt} *= $node_opts{node_cnt};
	}
...
#		}
	...
	if ($out_path) {
		$command .= " -o $out_path";
	        ...
	        $ENV{'SBATCH_OUTPUT_FILE'} = "%x.o%A";
		$ENV{'SBATCH_OUTPUT_FILE'}.= "-%a" if $array;
	}
...
$command .= " -G$node_opts{gpu_cnt}" if $node_opts{gpu_cnt};
...
sub parse_node_opts {
	...
	foreach my $part (@parts) {
		...
		foreach my $sub_part (@sub_parts) {
			if($sub_part =~ /ppn=(\d+)/ || $sub_part =~ /gpus=(\d+)/) {
				next;
			} elsif($sub_part =~ /^(\d+)/) {
				$opt{node_cnt} += $1;
			} else {
